...
|어노테이션|기능|
|---------|----|
|@Test|메서드가 테스트 메서드임을 나타낸다.<br>`JUnit4`의 `@Test`과 달리,<br>JUnit Jupiter(JUnit5)전용으로 작동하기 때문에 attribute를 선언하지 않는다.<br>어노테이션을 사용한 메서드는 상속되며 오버라이딩 가능하다.|
|@ParameterizedTest|메서드가 매개 변수가 있는 테스트 메서드임을 나타낸다.<br>어노테이션을 사용한 메서드는 상속되며 오버라이딩 가능하다.|
|@RepeatedTest|메서드가 반복 테스트를 위한 테스트 템플릿임을 나타낸다.<br>어노테이션을 사용한 메서드는 상속되며 오버라이딩 가능하다.|
|@TestFactory|메서드가 동적 테스트를 위한 테스트 팩토리임을 나타낸다.<br>어노테이션을 사용한 메서드는 상속되며 오버라이딩 가능하다.|
|@TestTemplate|메서드가 등록 된 공급자가 반환한 호출 컨텍스트 수에 따라,<br>여러 번 호출되도록 설계된 테스트 케이스의 템플릿임을 나타낸다.<br><br>어노테이션을 사용한 메서드는 상속되며 오버라이딩 가능하다.|
|@TestMethodOrder|어노테이션이 달린 테스트 클래스에 대한 테스트 메서드 실행 순서를 구성하는 데 사용된다.<br>JUnit 4의 @FixMethodOrder<br>상속 가능하다.|
|||
|||
